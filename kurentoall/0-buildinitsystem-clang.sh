...
#extract msys2-base-x86_64-20260611.tar.xz/msys64 to x:/dev/tools/mingw/msys64
#open msys64\clang64.exe
...
#call pacman -Syuu
#close the shell
#open msys64\clang64.exe again
#call pacman -Syuu again
###### now it has all basic tools updated
#call /x/dev/estos-kurento-scripts/kurentoall/0-buildinitsystem-clang.sh to install the basic tools for Kurento
#pack msys64-clang.zip -> remove all paths in /home and empty /etc/hosts
###### with existing prepared msys64-clang.zip from the tools repository x:\dev\tools\mingw\
#"C:\Program Files\7-Zip\7z.exe" x -y msys64-clang.zip -> extract in x:/dev/tools/mingw
#call pacman -Syuu
######
...
pacman -S --noconfirm mingw-w64-clang-x86_64-meson
pacman -S --noconfirm mingw-w64-clang-x86_64-clang
pacman -S --noconfirm mingw-w64-clang-x86_64-cmake
# pacman -S --noconfirm cmake
# C:\Program Files\CMake\bin\cmake.exe 4.2.0 must be installed
# pacman -S --noconfirm mingw-w64-clang-x86_64-indent
# pacman -S --noconfirm mingw-w64-clang-x86_64-astyle -> braucht astyle 2.06
...
pacman -S --noconfirm mingw-w64-clang-x86_64-pango

#for kurento
pacman -S --noconfirm mingw-w64-clang-x86_64-boost
pacman -S --noconfirm mingw-w64-clang-x86_64-libsigc++
# -> problem - conflict with glib from gstreamer Installation -> should be installed bevore Gstreamer Installation
pacman -S --noconfirm mingw-w64-clang-x86_64-glibmm
# uuid add /usr/lib/pkgconfig
pacman -S --noconfirm libutil-linux-devel
pacman -S --noconfirm mingw-w64-clang-x86_64-libvpx
pacman -S --noconfirm mingw-w64-clang-x86_64-libevent
pacman -S --noconfirm make
pacman -S --noconfirm mingw-w64-clang-x86_64-gdb
pacman -S --noconfirm mingw-w64-clang-x86_64-libsoup3
...
#pacman -Q mingw-w64-clang-x86_64-boost mingw-w64-clang-x86_64-boost-libs -> Status
#pacman -R mingw-w64-clang-x86_64-astyle
...
clang++ -g test.cpp -o test.exe
clang++ -fsanitize=address -g test.cpp -o test.exe
https://drmemory.org/
drmemory -- ./test.exe
clang++ -fsanitize=address -g -O0 test.cpp -o test.exe
clang++ -fsanitize=address -fsanitize=leak -g test.cpp -o test.exe
