...
clean_kmswindows_runtime_dlls()
{
	if [ -d "$TARGET_DIRECTORY/bin" ]; then
		find "$TARGET_DIRECTORY/bin" -maxdepth 1 -name '*.dll' -delete || true
	fi
	if [ -d "$TARGET_DIRECTORY/lib/kurento/modules" ]; then
		find "$TARGET_DIRECTORY/lib/kurento/modules" -maxdepth 1 -name '*.dll' -delete || true
	fi
	if [ -d "$TARGET_DIRECTORY/lib/gstreamer-1.0/kurento" ]; then
		find "$TARGET_DIRECTORY/lib/gstreamer-1.0/kurento" -maxdepth 1 -name '*.dll' -delete || true
	fi
	if [ -d "$TARGET_DIRECTORY/lib/gstreamer-1.0" ]; then
		find "$TARGET_DIRECTORY/lib/gstreamer-1.0" -maxdepth 1 -name '*.dll' -delete || true
	fi
}
...
sync_mingw_dll_deps()
{
	...
	for _dll in "$TARGET_DIRECTORY/lib/gstreamer-1.0"/*.dll \
		...
		"$TARGET_DIRECTORY/bin"/uc-media-server.exe
	do
		if [ -f "$_dll" ]; then
			_sync_one "$_dll" 0
		fi
